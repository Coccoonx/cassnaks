...
import co.wouri.libreexchange.uis.recipient.viewholders.RecipientViewHolder;
...
//            holder.leftImageView.setImageBitmap(recipientsItem.getImageUri());
        ...
        ((ImageView) holder.rightViewDelete).setColorFilter(Color.argb(255, 0, 0, 0));
        ((ImageView) holder.rightViewEdite).setColorFilter(Color.argb(255, 0, 0, 0));
    ...
    public void onItemDismiss(int position) {
        if (position != -1 && position < recipients.size()) {
            ...
            ProfileManager.deleteRecipient(recipients.remove(position));
